<?php
// config.php — файл с конфигурацией AmoCRM
return [
    'sub_domain' => 'testalexeybyrakov',
    'client_id' => '1e5eb9e5-db35-4e4b-a515-dcae731b7ee6',
    'client_secret' => '5STZ4APeOTE2CI9hofYEu9mwrgdDynVPF7aLkqKIIwYxht7ARX3xCLJoOrAxYgXs',
    'code' => '',
    'redirect_url' => 'https://testalexeybyrakov.amocrm.ru',

    // ОБЩИЕ ПАРАМЕТРЫ
    'pipeline_id' => 10362662,          // ID воронки "Воронка"
    
    // СТАДИИ
    'application_stage_id' => 81971238, // Стадия "Заявка"
    'waiting_stage_id' => 81971194,     // Стадия "Ожидание клиента" (используется в обеих задачах)
    'client_confirmed_stage_id' => 81971198, // Стадия "Клиент подтвердил"
    
    // НАСТРОЙКИ БЮДЖЕТА
    'budget_threshold' => 5000,         // Порог для MoveLeads
    'copy_budget_value' => 4999         // Точное значение для CopyLeads
];
